...
from tensorflow.python.keras.layers import Dense, LSTM, Conv1D, Embedding, Flatten
# Embedding 인풋 에서 많이 쓰인다 ( 인베딩 인풋 ) 파라미터 연산은 단어사전의 갯수 + 나가는 출력값의 갯수 
# 인베딩을 하면 원핫을 안해도 넣어진다 

model = Sequential()      #  14 , 5 ( 원핫이 안되어있는 수치의 shape )

                     # 단어사전의 갯수 # 아웃풋 노드의 갯수는 본인이 정하는 것                            
model.add(Embedding(input_dim=31, output_dim=10, input_length=5)) # 이렇게 나온 수치는 통상 3차원으로 나옴 ( 인베딩이 벡터화 시켜준다 ) 검색해서 정리할것 
...
model.add(Flatten()) # 여기서 3차원을 죄다 2차원으로 변경 후 모델링 가능 
model.add(Dense(10, activation='relu')) # 참 거짓을 밝히기에 sigmoid 
model.add(Dense(10, activation='relu')) # 참 거짓을 밝히기에 sigmoid 
model.add(Dense(10, activation='relu')) # 참 거짓을 밝히기에 sigmoid 
...
model.fit(pad_x, labels, epochs=15, batch_size=10)
