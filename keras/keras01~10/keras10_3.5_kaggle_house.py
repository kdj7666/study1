...
lbl = LabelEncoder()
cat_col = list(path+'train.csv'.str(float)[train_set.dtypes == 'object'].index)
cat_col = list(path+'test.csv'.str(float)[test_set.dtypes == 'object'].index)
# train_set = tf.reshape(train_set, ['train.csv'])
# test_set = tf.reshape(test_set, ['test.csv'])
print(train_set.shape + str(train_set.get_shape()))
print(test_set.shape + str(test_set.get_shape()))
for col in cat_col:
        train_set[col] = lbl.fit_transform(train_set[col.values])
        test_set[col] = lbl.fit_transform(test_set[col.values])
# print(train_set.shape + str(train_set.get_shape()))
# print(test_set.shape + str(test_set.get_shape()))
