...
model.add(SimpleRNN(units=24,return_sequences=True, input_shape=(3,1)))  # [batch, timesteps, feature]
...
# (None, 64) 64개를 주었어 
model.add(Bidirectional(SimpleRNN(7)))
model.add(Dense(3,activation='swish'))     # 윗줄 3차원 받을때는 2차원 ( 물어볼것 )
model.add(Dense(1))
model.summary()
