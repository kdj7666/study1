...
model.add(Dense(400, activation='sigmoid')) # activation 활성화 함수 이것으로 인해 결과값이 엄청좋아지고 안좋아진다 필수 ( 다시 공부할것 시그모이드 포함 )
model.add(Dense(300, activation='relu'))    #  레이어를 한정시키며 펑 터지는것을 방지한다 ( 찾아볼것 )
model.add(Dense(350, activation='relu'))
model.add(Dense(350, activation='relu'))
model.add(Dense(350, activation='relu'))# linear 선형  
...
earlystopping = EarlyStopping(monitor='val_loss', patience=200, mode='min', verbose=1, # mode='min'뿐아니라 max도 있음  디폴드값 찾아볼것 모르면 오토 
...
# cpu  걸린시간 :  16.857855081558228 1500 320 

# acc.score :  0.9298245614035088
# gpu  걸린시간 :  30.233569145202637 1500 
