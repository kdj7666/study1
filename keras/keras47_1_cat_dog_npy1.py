...
xy_train = train_datagen.flow_from_directory(
    ...
    shuffle=True,
)   # Found 8005 image belonging to 2 classes.
...
xy_test = test_datagen.flow_from_directory(
    ...
    shuffle=True,
)   # Found 2023 image belonging to 2 classes.
...
# np.save('d:/study_data/_save/_npy/keras47_1_train_x(cat_dog).npy', arr=xy_train[0][0])
# np.save('d:/study_data/_save/_npy/keras47_1_train_y(cat_dog).npy', arr=xy_train[0][1])
# np.save('d:/study_data/_save/_npy/keras47_1_test_x(cat_dog).npy', arr=xy_test[0][0])
# np.save('d:/study_data/_save/_npy/keras47_1_test_y(cat_dog).npy', arr=xy_test[0][1])
# # 넌파이 파일로 저장한다 넌파일수치로 저장이 됨

x_train = np.load('d:/study_data/_save/_npy/keras47_1_train_x(cat_dog).npy')
y_train = np.load('d:/study_data/_save/_npy/keras47_1_train_y(cat_dog).npy')
x_test = np.load('d:/study_data/_save/_npy/keras47_1_test_x(cat_dog).npy')
y_test = np.load('d:/study_data/_save/_npy/keras47_1_test_y(cat_dog).npy')
...
# hist = model.fit_generator(x_train,y_train, epochs=10, steps_per_epoch=10,    
#                          # 스텝 펄 에포 ( 통상적으로 batch= 160/5 = 32)  # 훈련 배치 사이즈가 32가 넘어서도 돌아가긴한다 추가적 환경 제공 가능
#                     validation_data=y_test,                    # 발리데이션 범주를 테스트로
...
# loss :  1.538786875698861e-08
# val_loss :  11.172911643981934
# val_accuracy :  0.0
# accuracy :  1.0
# 걸린시간 :  9.862720727920532
