...
model = Sequential()
model.add(Dense(100, activation='linear', input_dim=30))    # linear = y=wx+b 자체이기 때문에 상관없다 / relu(y=wx+b)sigmoid 도 동일 ( 찾아볼것 ) relu마지막은 불가능 음수값을 차단하기 때문
...
earlystopping = EarlyStopping(monitor='val_loss', patience=120, mode='auto', verbose=1, # mode='min'뿐아니라 max도 있음  디폴드값 찾아볼것 모르면 오토 
              restore_best_weights=True)  # < - 검색해서 정리할것 (파라미터를 적용을 시켯다 내가 하고싶은대로)
...
# 과제 
# activaion : sigmoid , relu linear 넣고 돌리기 
# metrics 추가
# EarlyStopping
# 성능비교
# 감상문 , 느낀점 2줄 이상 
